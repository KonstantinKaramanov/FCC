function rot13(str) {
  ...
  for(let i = 0; i < str.length; i++) {
  	if(str[i].match(regex)) { //if the ith value of str is a character A-Z:
      ...
      if(newIndex > 26) { //if the shifted value goes past the letters of the alphabet...
      ...
      else if(newIndex == 26) { //should combine this else if with the if above:  newIndex >= 26.
      	newStr += alphabet[0];
      }
      else { //if under 26, then just add character to new string
      	newStr += alphabet[newIndex];
      }
    }
  	else { //if this isn't a character A-Z, then just pass it on to the new string
    	newStr += str[i];
    }
  }
  console.log(newStr);
